...
- [x] Run `/impeccable` setup to establish component hierarchy and inspection UX before building UI.
- [x] Decision Lineage Inspector modal/drawer opens upon clicking any historical trade or decision point in the timeline.
- [x] Displays exact historical inputs: OHLCV bar window, point-in-time indicators (RSI, SMA, MACD), and Benzinga news headlines filtered to $\le T$.
- [x] Displays exact LLM prompt rendered for the decision, raw model completion string, and parsed Zod signal contract.
- [x] Visualizes debate transcript showing individual specialist votes, consensus check outcome, and synthesis arguments (if debate occurred).
- [x] Telemetry HUD displays total token expenditure, estimated API cost in USD, median inference latency in ms, and degradation/fallback rate.
- [x] Unit tests verify inspector renders correct lineage data for selected decision points without crashing on edge cases.
