...
def render(
    ...
    try:
        ...
        print("Run: cd .agents/skills/excalidraw-diagram/references && python3 -m venv .venv && .venv/bin/pip install playwright && .venv/bin/playwright install chromium", file=sys.stderr)
        sys.exit(1)
    ...
    with sync_playwright() as p:
        try:
            ...
            if "Executable doesn't exist" in str(e) or "browserType.launch" in str(e):
                ...
                print("Run: cd .agents/skills/excalidraw-diagram/references && .venv/bin/playwright install chromium", file=sys.stderr)
                sys.exit(1)
            raise
        ...
        # Listen for console logs, errors, and request failures
        ...
        page.on("requestfailed", lambda req: print(f"BROWSER REQUEST FAILED: {req.url} - {req.failure if req.failure else 'No error text'}", file=sys.stderr))
